...
# GoB Prow
PROW_GOB_ROOT="${TESTINFRA_ROOT}/prow/gob"
PROW_GOB_BUCKET="gob-prow"

bazel test config/tests/jobs:go_default_test --test_arg="-config=${PROW_GOB_ROOT}/config.yaml" --test_arg="-job-config=${PROW_GOB_ROOT}/config" --test_arg="-gubernator-path=${GUBERNATOR_URL}" --test_arg="-bucket=${PROW_GOB_BUCKET}" --test_arg="-k8s-prow=false" || EXIT=$?
